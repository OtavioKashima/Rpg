...
export default function OnePieceArena() {
  ...
  // CORREÇÃO 1: Cursor ajustado sem o 'public/' e com fallback 'crosshair'
  const [cursorStyle, setCursorStyle] = useState("url('/cursor-hand.png'), crosshair");
  const [isShopOpen, setIsShopOpen] = useState(false);
  const [hudData, setHudData] = useState({ berris: 0, level: 1, xp: 0, maxXp: 100, wave: 1, killsThisWave: 0, killsNeeded: 10, hp: 100, maxHp: 100, className: '' });
  ...
  const serverWorldRef = useRef({ players: {}, enemies: {}, boss: null });
  const visualEffectsRef = useRef([]);

  const imagesRef = useRef({
    players: { cortante: null, atirador: null, especialista: null },
    enemies: { marineMelee: null, bigmom: null }, // Adicionado slot para a Big Mom
    background: null
  });
  ...
  useEffect(() => {
    ...
    socketRef.current.on('playerHit', (damage) => {
      const pState = gameState.current.player;
      // Se o personagem acabou de tomar dano, ele ignora os outros golpes (Fica invencível)
      if (pState.hitTimer > 0) return;

      pState.hp -= damage;
      pState.hitTimer = 45; // Fica imortal (e piscando em vermelho) por quase 1 segundo
    });
  ...
  useEffect(() => {
    ...
    const loadImages = () => {
      ...
      imagesRef.current.enemies.bigmom = load('/bigmom.jpg'); // Carrega a Big Mom
    };
    loadImages();
    ...
    const handleContextMenu = (e) => {
      ...
      isAwaitingAttackClick = false; setCursorStyle("url('/cursor-hand.png'), crosshair");
      let targetFound = false;
      const world = serverWorldRef.current;

      // Tenta mirar no boss primeiro
      if (world.boss && getDist(e.clientX, e.clientY, world.boss.x, world.boss.y) <= world.boss.radius + 5) {
        player.targetId = 'bigmom'; targetFound = true;
      }

      // Se não clicou no boss, procura inimigos normais
      if (!targetFound) {
        for (let eId in world.enemies) {
          if (getDist(e.clientX, e.clientY, world.enemies[eId].x, world.enemies[eId].y) <= world.enemies[eId].radius + 5) {
            player.targetId = eId; targetFound = true; break;
          }
        }
      }
    ...
    const handleMouseDown = (e) => {
      if (e.button === 0 && isAwaitingAttackClick && !gameState.current.isShopOpen) {
        ...
        // Verifica o boss
        if (world.boss) {
          const dist = getDist(e.clientX, e.clientY, world.boss.x, world.boss.y);
          if (dist < minDistance) { minDistance = dist; closestId = 'bigmom'; }
        }

        // Verifica inimigos
        for (let eId in world.enemies) {
          const dist = getDist(e.clientX, e.clientY, world.enemies[eId].x, world.enemies[eId].y);
          if (dist < minDistance) { minDistance = dist; closestId = eId; }
        }
    ...
    function animate(timestamp) {
      ...
      // Lógica de ataque aprimorada (Suporta o Boss)
      if (player.targetId !== null) {
        let target = player.targetId === 'bigmom' ? world.boss : world.enemies[player.targetId];

        if (!target) { player.targetId = null; }
        else {
          const distToTarget = getDist(player.x, player.y, target.x, target.y);
          if (distToTarget <= pState.range + target.radius) { // Ajuste de range para bater em alvos grandes
      ...
      for (let pId in world.players) {
        ...
        ctx.beginPath(); ctx.arc(other.x, other.y, other.radius || 20, 0, Math.PI * 2);
        ctx.fillStyle = other.color || '#94a3b8'; ctx.fill();
        ctx.lineWidth = 2; ctx.strokeStyle = '#fff'; ctx.stroke();
      }

      // DESENHA O BOSS (BIG MOM)
      if (world.boss) {
        let boss = world.boss;
        let bossImage = imagesRef.current.enemies.bigmom;
        if (bossImage && bossImage.isReady) {
          ctx.drawImage(bossImage, boss.x - boss.radius, boss.y - boss.radius, boss.radius * 2, boss.radius * 2);
        } else {
          ctx.beginPath(); ctx.arc(boss.x, boss.y, boss.radius, 0, Math.PI * 2);
          ctx.fillStyle = 'purple'; ctx.fill();
        }
        // Barra de Vida Centralizada da Big Mom
        ctx.fillStyle = 'black'; ctx.fillRect(boss.x - 100, boss.y - boss.radius - 20, 200, 15);
        ctx.fillStyle = 'red'; ctx.fillRect(boss.x - 100, boss.y - boss.radius - 20, (boss.hp / boss.maxHp) * 200, 15);
      }
      ...
      for (let eId in world.enemies) {
        ...
        ctx.fillStyle = 'black'; ctx.fillRect(enemy.x - 15, enemy.y - enemy.radius - 10, 30, 4);
        ctx.fillStyle = '#ef4444'; ctx.fillRect(enemy.x - 15, enemy.y - enemy.radius - 10, 30 * (enemy.hp / enemy.maxHp), 4);
      }
      ...
      if (pState.hp <= 0) {
        if (socketRef.current) socketRef.current.emit('playerDeath'); // Avisa o server na hora!
        setAppState('GAMEOVER');
        return;
      }

      // Atualiza a HUD com as informações da Wave vindo do Servidor
      frameCount++;
      if (frameCount % 15 === 0) {
        setHudData({
          berris: pState.berris, level: pState.level, xp: pState.xp, maxXp: pState.maxXp,
          wave: world.wave || 1, killsThisWave: world.killsThisWave || 0, killsNeeded: world.killsNeeded || 10,
          hp: Math.floor(pState.hp), maxHp: pState.maxHp, className: pState.name
        });
      }
    }
    animate(0);
  ...
  return (
    <div style={{ margin: 0, padding: 0, overflow: 'hidden', backgroundColor: '#0f172a', height: '100vh', width: '100vw', fontFamily: 'sans-serif', cursor: cursorStyle }}>
      ...
      {appState === 'PLAYING' && (<> <div style={{ position: 'absolute', top: 10, left: 10, color: 'white', backgroundColor: 'rgba(15, 23, 42, 0.85)', padding: '15px', borderRadius: '8px', zIndex: 50, pointerEvents: 'none' }}> <h2 style={{ margin: 0, color: CLASSES[gameState.current.player.classId]?.color }}>{hudData.className}</h2> <div style={{ backgroundColor: '#333', width: '220px', height: '22px', marginTop: '10px' }}><div style={{ backgroundColor: '#ef4444', width: `${Math.max(0, (hudData.hp / hudData.maxHp) * 100)}%`, height: '100%' }} /></div> <p style={{ margin: '5px 0 15px 0', fontSize: '14px', fontWeight: 'bold' }}>{hudData.hp} / {hudData.maxHp} HP</p>
        <p style={{ margin: '5px 0', color: '#10b981', fontWeight: 'bold', fontSize: '16px' }}>🌊 Wave: {hudData.wave === 10 ? 'BOSS FIGHT!' : `${hudData.wave} / 10`} ({hudData.killsThisWave}/{hudData.killsNeeded})</p>
        <p style={{ margin: '5px 0', color: 'gold', fontWeight: 'bold' }}>💰 Berris: {hudData.berris}</p> <p style={{ margin: '5px 0', color: '#38bdf8', fontWeight: 'bold' }}>⭐ Nível: {hudData.level} (XP: {hudData.xp}/{hudData.maxXp})</p> <p style={{ margin: '15px 0 0 0', fontSize: '12px', color: '#94a3b8' }}>Aperte <b>'B'</b> para Mercado Negro</p> </div>
